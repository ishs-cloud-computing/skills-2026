...
- 지급 계정은 PowerUserAccess 수준 IAM 유저다(root 미지급). 사전 제공 리소스 보호용 명시적 Deny 규칙이 붙을 수 있다 (2026-07-31 협의회).
- 1·2과제만 사전 공개되며 이 중에서 출제된다.
- 당일 변경(채점 30% 이내)은 기존 문제 수정이 아니라 **추가 문제** 방향이 원칙이다. 2과제는 최대 2개 모듈 추가(최대 6개, 추가 시간 없음), 세트 선정은 추첨된 1인 세트의 4모듈 전체 (2026-07-31 협의회).
- EKS 채점은 CloudShell kubectl 기준. 접근 방식은 자유이나 채점 중에는 `aws eks update-kubeconfig` 1회만 허용되므로 채점 전 CloudShell 사전 구성이 필수다.

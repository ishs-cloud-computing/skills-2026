...
- 채점지 11-3 구체화: 필수 패널 16종을 본문 명시 + "모든 메트릭 **및 로그**는 빈값이 없어야" +
  ...
  패널 제목은 `Status Codes` 를 유지한다 — 채점지 사진(mark.pdf p.14)의 실제 제목이다.
  누락 패널은 없었다
...
- 범위 밖 보강 2건(사용자 승인): `dashboard.json` Status Code 카운터 3종에 `or vector(0)` 추가
  ...
  과 동일 패턴). CloudWatch ELB 4XX/5XX 타깃은 **유지**한다 — 사진 근거는 아래 2026-08-22 결정 로그
...
### 2026-08-22 Status Codes 패널 — ELB 4XX/5XX 타깃 유지 + 제목 복수형 복원 (사진이 정본)
- 맥락: PR #137 코드리뷰가 "ELB 4XX/5XX 는 구조적으로 빈값이라 11-3 의 `모든 메트릭 및 로그는
  빈값이 없어야합니다` 를 위반한다"고 지적했다. 부하 파드(`error-gen` → `/nonexist`)의 4xx 는
  앱이 만들므로 `HTTPCode_Target_4XX_Count` 이지 `HTTPCode_ELB_4XX_Count` 가 아니다 — 지적 자체는 맞다.
- 근거(신규): `pdfimages -f 14 -l 14 set-03/task-1/mark.pdf` 로 채점지 사진을 뜯어 확인했다.
  **공식 참조 대시보드 자체가** 범례에 2XX/4XX/5XX/**ELB 4XX/ELB 5XX** 5종을 달고 있고,
  실제로 그려진 선은 2XX 하나뿐이다. 즉 "빈값이 없어야 한다"가 시리즈 단위였다면 정본 사진이
  자기 규칙에 걸린다. 판정 단위는 **패널**(No data 금지)이지 시리즈가 아니다.
- 채택: 타깃 D/E 유지. 삭제하면 "채점지 사진과 일치" 쪽을 확실히 잃고, 얻는 건 성립하지 않는 해석뿐이다.
- 채택: 패널 제목을 `Status Code` → **`Status Codes`** 로 되돌린다. 같은 사진의 제목이 복수형이다.
  11-3 글머리표의 `Status Code - 어플리케이션 응답 코드별 응답 수` 는 "구성 요소가 포함되어 있는지"
  체크리스트라 제목 문자열이 아니다. 본문이 먼저 지시하는 건 사진 대조다.
- 기각: 타깃 D/E 삭제(코드리뷰 제안) — 위 근거로 전제가 깨진다.
- 기각: ELB 타깃을 `HTTPCode_Target_*` 로 교체 — 범례 라벨이 `ELB 4XX` 라 사진과 어긋난다.
  사진의 정본 구현도 ELB 지표를 그대로 두고 데이터가 안 나오는 상태로 제출됐다.

### 2026-08-21 [번복] mark.sh 를 정본 그대로 되돌린다 — 바로 아래 항목을 폐기한다
- 번복 사유: **채점 스크립트는 정본 전사본이지 우리가 최적화할 대상이 아니다.** 리허설 시간을
  아끼려고 정본과 다른 흐름을 만들면, 정작 채점자가 실제로 도는 경로를 리허설이 검증하지 못한다.
  아래 항목의 "리허설 도구니까 최적화해도 된다"는 전제 자체를 기각한다
- 폐기한 것: `start_load_pods()`(파드 생성을 5-5 직후로 이관) / `wait_for_alerts()`
  (`ALERTS{alertstate="firing"}` 폴링으로 `sleep 180` 대체) / `GRAFANA_CRED` / 상단 "정본과
  의도적으로 다른 지점" 주석 블록
- 복원한 것: 파드 6종 생성 블록 + `sleep 180` 을 **11-1·11-3 양쪽에**, 11-3 의 `GRAFANA_LB` 재할당.
  `error-gen`·`latency-gen` 의 URL 도 정본 표기 `http://'"$SVC_IP"'/…` 그대로 둔다
  (bea5392 가 `${SVC_IP}` 로 "고쳤던" 것을 되돌렸다)
- 대가: 리허설마다 `sleep 180` × 2 = 6분이 다시 든다. 정본과 같은 값을 치르는 게 맞다고 판단했다
- 유지한 것(정본 전사가 맞는 부분이라 되돌리지 않았다): 11-3 필수 구성 요소 16종 열거,
  "모든 메트릭 및 로그는 빈값이 없어야 한다", 로그 예시 `INFO {"level":"INFO",…}`,
  11-4 의 "HighLatency 는 채점과 무관", 5-5 의 `wsc2026-eks-cluster`
- 검증: `mark.sh` 와 `mark.md` 의 `kubectl run` 12개가 명령 단위로 완전 일치함을 확인했다
  (`mark.md` 는 가독성 때문에 줄을 나눠 쓰고 `mark.sh` 는 정본대로 `;` 로 이었을 뿐이다)

### 2026-08-21 [기록] 11-4 알람 발화 타이밍이 실채점에서 경계선이다 — 손댈 수 있는 레버가 없다
- 정본 흐름: 11-1 에서 파드 생성 후 `sleep 180`, 11-3 에서 다시 생성하지만 `AlreadyExists` 로
  조용히 실패하고 `sleep 180` 만 다시 돈다 → 파드 생성부터 11-4 확인까지 **약 360초**
- `PodCrashLooping` 은 재시작 4회가 필요하고 CrashLoopBackOff 백오프가 10+20+40+80 ≈ 150초,
  거기서 `for: 3m` 이라 발화가 t+330초. 스크레이프·평가 지연까지 더하면 **약 6분**이라 360초와
  거의 같다. `PodHighCPU`(`for: 3m`, `rate[2m]`)도 약 5분으로 여유가 크지 않다
- `stress-mem` 도 같은 계열이다 — `--vm-bytes 60M` / limit `64Mi` = 89.4% 로 `PodHighMemory`
  임계 `> 0.9` 바로 아래고, working set 이 임계를 넘으면 `restartPolicy: Never` 라 OOMKill 후
  Failed 로 끝나 3분을 못 채운다
- 손댈 수 없는 이유: 임계치는 과제지가 문자 그대로 규정한다(`재시작 횟수가 3회를 초과한 상태가
  3분 이상`, `메모리 사용률 90%를 3분 이상`). `prometheus-rules.yaml` 을 완화하면 과제지 위반이다.
  파드 스펙도 정본 값이라 `mark.sh` 에서 못 바꾼다
- README step 9-2 의 파드 삭제도 그대로 둔다 — 남기면 채점자 `mark.sh` 의 `kubectl run` 이
  `AlreadyExists` 로 죽어 `PodHighMemory` 가 아예 안 뜬다. 지우는 쪽이 손해가 작다
- 결론: **당일 11-4 가 4/5 로 보일 수 있다는 것만 알고 간다.** 수동 채점 항목이므로 채점자에게
  "재시작 카운트가 아직 3을 못 넘었다"를 `kubectl get pod crash-test` 로 보여줄 수 있게 준비한다

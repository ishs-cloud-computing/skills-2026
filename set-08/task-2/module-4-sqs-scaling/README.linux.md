...
sleep 45; kubectl get pods -n skills-sqs -l app=sqs-worker      # 큐가 비었으면 0개여야 한다
```
...
`mark2-4.sh`는 4-6의 `sleep 60`×3(위 준비를 건너뛰면 kubectl 설치도) 때문에 실측 **약 11분** 걸린다(다른 모듈 채점은 각 1~3분).

...
# ---------------------------------------------------------------------------
# IAM (과제지 4-2·4-5)
# - skills-sqs-keda-policy       : KEDA 가 SQS 큐 길이를 조회 (IRSA)
# - skills-sqs-worker-policy     : worker 앱의 메시지 수신·삭제 (IRSA)
# - skills-sqs-karpenter-policy  : Karpenter 컨트롤러 권한 (IRSA)
...
resource "aws_iam_policy" "keda" {
  name = "${var.name_prefix}-keda-policy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
# ----- App: SQS 메시지 수신·삭제 정책 (IRSA, 과제지 "수신·삭제 권한을 적절히 부여") -----

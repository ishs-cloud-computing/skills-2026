...
# - public  : IGW (App EC2 + LoadBalancer(NLB) 배치)
# - private : NAT (EKS NodeGroup 배치)
# Loki/Grafana NLB(internet-facing)가 public 서브넷에 생성되도록
# kubernetes.io/role/elb 태그를 부여한다.
# ---------------------------------------------------------------------------
...
resource "aws_eip" "nat" {
  ...
  domain   = "vpc"
  tags     = { Name = "wsc-logging-nat-eip-${each.key}" }
}
...
resource "aws_nat_gateway" "this" {
  ...
  tags          = { Name = "wsc-logging-nat-${each.key}" }

...
# --- 인자 확인 ---
if [ $# -lt 1 ]; then
  echo -e "${RED}[ERROR] 비번호를 입력하세요.${NC}"
  echo "사용법: bash grade_task1.sh <비번호>"
  exit 1
fi
...
echo ""
echo -e "${BOLD}  비번호(선수ID): ${YELLOW}${PLAYER_ID}${NC}"
...
# 1-3: 리소스 명명 규칙 확인 (1점)
echo -e "\n${BOLD}[1-3] 리소스 이름에 비번호(선수ID) 접두어 확인 (1점)${NC}"
...
if [ "$NAME_OK" -ge 3 ]; then
  ...
  fail "1-3" "비번호(선수ID) 접두어 미적용 리소스 존재 (확인: VPC=$VPC_NAME, IGW=$IGW_NAME, RT=$RT_NAME)"
fi
...
# 3-2: latest 태그 이미지 (1.0점) + Linux/AMD64 (0.5점) = 1.5점
echo -e "\n${BOLD}[3-2] ECR latest 태그 이미지 확인 (1.0점) + Linux/AMD64 아키텍처 (0.5점)${NC}"
...
if echo "$LATEST_IMAGE" | grep -q "latest"; then
  pass "3-2 latest" "1.0" "latest 태그 이미지 존재"
else
...
# 4-5: CloudFront를 통해 GET /health HTTP 200 확인 (1점)
echo -e "\n${BOLD}[4-5] CloudFront를 통해 GET /health HTTP 200 확인 (1점)${NC}"

# CF_DOMAIN이 아직 설정되지 않은 경우 조회
...
if [ "$CF_DOMAIN" = "None" ] || [ -z "$CF_DOMAIN" ]; then
  fail "4-5" "CloudFront Distribution 조회 불가 (정적 호스팅 버킷: ${PLAYER_ID}-static-site)"
else
  HEALTH_CODE=$(curl -s -o /dev/null -w "%{http_code}" "https://${CF_DOMAIN}/health" 2>/dev/null || echo "000")
  if [ "$HEALTH_CODE" = "200" ]; then
    pass "4-5" "1" "GET https://${CF_DOMAIN}/health → HTTP $HEALTH_CODE"
  else
    fail "4-5" "GET https://${CF_DOMAIN}/health → HTTP $HEALTH_CODE (200 필요)"
  fi
fi
...
info "※ IaC(Terraform, CDK 등) 오픈소스 도구 사용은 직종설명서에 따라 허용됩니다."
...
if awk "BEGIN { exit !($TOTAL_SCORE >= 27) }"; then
  echo -e "  ${GREEN}${BOLD}우수${NC}"
elif awk "BEGIN { exit !($TOTAL_SCORE >= 21) }"; then

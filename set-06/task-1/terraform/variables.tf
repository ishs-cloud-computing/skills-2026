# 대회 당일 30% 변경 대비 — 이름·CIDR·리전·타입은 전부 변수 (plan.md §4)
...
variable "client_id_regex" {
  description = "WAF client_id 검증 정규식 (앵커 필수 — plan.md §3.10)"
  type        = string
  default     = "^[A-Za-z][A-Za-z0-9]*[0-9][A-Za-z0-9]*$"
}

variable "enable_ddb_write_deny" {
  description = "DynamoDB 쓰기 Deny 리소스 정책 활성화. 데이터 정리 시 일시 false (plan.md §3.4 함정)"
  type        = bool
  default     = true
}

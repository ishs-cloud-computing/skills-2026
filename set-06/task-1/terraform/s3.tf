# ---------------------------------------------------------------------------
# S3 (요구사항 10)
# - Bucket Name: gj2026-static-<비번호>
# - 모든 콘텐츠 파일은 버킷 루트에 업로드 (채점 6-1)
# - 업로드 시 CMK(alias/gj2026-s3-key) 로 자동 암호화 (채점 6-2)
# ---------------------------------------------------------------------------
...
# 제공된 정적 파일(index.html, main.jpeg)을 버킷 루트에 업로드한다.
# ./assets/ 에 제공자료를 그대로 배치한 뒤 적용. (key 에 prefix 없음 = 루트)

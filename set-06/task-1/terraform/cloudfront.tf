...
# ---------------------------------------------------------------------------
# CloudFront (NOTES.md §3.9) — 단일 엔드포인트, Origin 3개 / Behavior 4개
...
locals {
  # AWS Managed 캐시/오리진 요청 정책 고정 ID (NOTES.md §3.9 표)
...
resource "aws_cloudfront_vpc_origin" "alb" {
  vpc_origin_endpoint_config {
    # 주의: 배포에 연결된 동안 이 리소스는 이름 포함 완전 불변(update/delete 모두 409) —
    # 교체가 필요하면 behavior 에서 분리 → 교체 → 재연결 순서뿐이다 (NOTES.md §6 함정 34)
  ...
  # 배포(distribution)가 참조 중인 상태에서 destroy-then-create 는 409 로 실패한다 — 교체는 CBD 로만 가능
  lifecycle {
...
# ALB/Lambda behavior 에 붙이면 /reservation 이 재작성되어 깨진다 (NOTES.md §3.9)

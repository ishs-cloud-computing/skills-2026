# DB 초기화 런북 (Linux bash 버전)

[db/README.md](README.md)의 PowerShell 판과 동일 절차의 bash 판. 메인 런북
[../README.linux.md](../README.linux.md) STEP 7에서 진입한다. **cwd = task-3/** 기준, 로컬 bash.

- 모든 작업은 **직결 엔드포인트**(`db_endpoint`)로 한다 — 프록시 경유 대량 적재는 세션이 프록시에 피닝되어 풀링을 깨뜨린다. 앱만 프록시(`db_proxy_endpoint`)를 쓴다.
- 정적 SQL은 이 디렉토리의 `.sql` 파일을 stdin(`<`)으로 주입, 비밀번호가 들어가는 문장(ALTER USER)만 문자열 — 시크릿을 git에 넣지 않기 위함.
- mysql 클라이언트는 클러스터 안에서 `kubectl run`으로 띄운다(로컬 mysql 불필요). 따라서 **STEP 5 노드풀이 Ready여야 한다** — 첫 `kubectl run`이 노드 생성을 트리거해 ~2분 걸린다.
- **순서 주의**: 1·2번(스키마·ALTER USER)은 즉시 끝나고 앱이 붙기 위한 전제조건이다. 3번 dump 적재(50만행)가 느린 쪽이므로 **2번이 끝나면 README STEP 8(앱 배포)을 새 터미널에서 시작**하고 3번을 이어서 돌린다.

## 0. 전제

```bash
# ── 로컬 bash ──
# README STEP 0에서 이미 수행: $DB_PASSWORD (terraform.tfvars의 db_password와 동일)
...
kubectl run db-init --rm -i --restart=Never \
  ...
  mysql -h "$DB_HOST" -uadmin "-p$DB_PASSWORD" dev < db/01-schema.sql
```
...
echo "ALTER USER 'admin'@'%' IDENTIFIED WITH mysql_native_password BY '$DB_PASSWORD';" |
  ...
  --image=public.ecr.aws/docker/library/mysql:8.0 -- \
  mysql -h "$DB_HOST" -uadmin "-p$DB_PASSWORD" dev
```

> 여기서 **README STEP 8(앱 배포)을 새 터미널에서 시작**한다. 스키마·인증이 끝났으므로 앱은 빈 테이블에 정상 연결되고, 데이터는 T+60 트래픽 전에만 있으면 된다. 노드 생성·이미지 pull·파드 기동·TG 등록·ALB 헬스체크가 아래 dump 적재와 겹쳐 ~2–3분 회수된다.
...
kubectl run db-load --rm -i --restart=Never \
  --image=public.ecr.aws/docker/library/mysql:8.0 -- \
  mysql -h "$DB_HOST" -uadmin "-p$DB_PASSWORD" dev < load_user.dump
```

느리면(kubectl stdin은 빠른 파이프가 아니다) dump를 S3에 올리고 파드가 직접 당기는 방식으로 전환한다. 크기를 모르는 지금 미리 만들 이유는 없다.
...
kubectl run db-index --rm -i --restart=Never \
  --image=public.ecr.aws/docker/library/mysql:8.0 -- \
  mysql -h "$DB_HOST" -uadmin "-p$DB_PASSWORD" dev < db/02-index.sql
```
...
kubectl run db-verify --rm -i --restart=Never \
  --image=public.ecr.aws/docker/library/mysql:8.0 -- \
  mysql -h "$DB_HOST" -uadmin "-p$DB_PASSWORD" dev <<'SQL'

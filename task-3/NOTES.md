...
### 2026-08-24 — stress 부하 특성 실측 (재구성 바이너리) → CPU limit 기각

- **측정**: 60 동시연결로 100초, 16,301 요청(약 163 rps).
- **결과**: stress 3m → **193m**, 같은 시점 product 9m · user 6m. 노드 169m(8%) → 599m(31%).
  HPA `stress 77%/60%` → 2 replicas (user 2% · product 3% 는 무반응). Karpenter 노드 1대 추가 후 회수.
- **`length` 는 부하 파라미터가 아니다**: 256 ~ 10,000,000 전 구간에서 서버 측 87~246µs 로 동일.
  부하는 rate 기반이고 요청당 약 1.2 millicore 다. 과제지 예시(`"length": 256`)를 보고 작업량
  파라미터로 짐작하기 쉬운데 아니다. 유휴(3 rps)에는 stress 가 셋 중 CPU 최저(3m)라 특성이 안 보인다.
- **이번 부하에서 user·product 는 안 다쳤다**: 노드가 31% 라 CFS 경합이 없었다. HPA·Karpenter 가
  포화 전에 퍼뜨린다. 위험은 **노드 부팅 창**에 한정된다 — nodeclaim 생성부터 Ready 까지 실측 약 2분.
- **한계 — 이건 작년 대회 바이너리를 재구성한 것이다.** 요청당 처리시간·millicore 는 당일 바이너리에서
  다시 재야 한다. 재측정 절차는 README STEP 12.
- **기각**: stress 에 CPU limit(800m) — 노드 부팅 창 약 2분 동안 stress 가 노드를 포화시켜 user·product
  의 0.2s 예산을 먹는 시나리오를 막으려 했고, 별도 브랜치(`task-3/stress-cpu-limit`)로 갈라 뒀다가
  폐기했다. 실측에서 노드가 31% 에 그쳐 방어할 경합이 실제로 관측되지 않았고, 800m 의 근거가 통째로
  재구성 바이너리 실측 하나뿐이라 당일 바이너리에서 그 숫자가 어긋나면 오히려 stress 를 스로틀링해
  performance 4점과 cost ratio 12점을 같이 건다. "CPU limit 금지" 원칙에 예외를 두지 않는다.
- **기각**: stress 에 podAntiAffinity — 부팅 창에서는 피할 노드가 아직 없어 무력하다. 실측 전에
  1순위로 꼽았다가 철회했다.
- **기각**: request 값 조정 — 올리면 스케일아웃이 늦어지고(60% × 500m = 300m) CFS weight 가 올라
  경합에서 stress 가 더 이긴다. 내리면 노드당 패킹이 빽빽해져 경합 밀도가 오른다. 양방향 다 악화라
  250m/512Mi 를 유지한다.
- **주의**: metrics-server 는 약 30초 평균이라 193m 은 피크를 과소평가했을 수 있다.

### 2026-08-24 — 부하 종료 후 1대 복귀에 7~9분이 걸리던 문제 → 회수 병렬화 + 한 스텝 감축

- **문제**: 유휴 1대라는 *바닥*은 이미 맞았는데(용량 검산 1600m/1930m, 12/17 pods) 스파이크가 끝나고
  1대로 돌아오는 *꼬리*가 길었다. 비용 ratio 는 실행 중인 EC2 를 주기적으로 세므로 이 구간이 그대로
  시간평균에 실린다. 내역은 HPA `scaleDown` 40s stabilization + 40s당 50% 로 10→1 에 4스텝(~160초),
  이어서 NodePool `budgets: nodes "1"` 이 빈 노드 6대를 한 대씩 회수(대당 40~60초).
- **채택**: `k8s/01-nodepool.yaml` budget `"1"` → `"50%"`, 앱 3종 HPA `scaleDown` Percent 50 → 100.
  Karpenter 는 파드가 다른 노드에 재배치 가능할 때만 노드를 지우므로 병렬 회수로 Pending 이 생기지
  않는다 — budget 이 막아주던 블라스트 반경이 여기서는 실익이 없다. 목표 꼬리 ~2분.
- **유지**: `stabilizationWindowSeconds: 40`. 부하가 되돌아올 때 flapping 을 막는 유일한 가드다.
  `scaleUp` 블록(stabilization 0 + 15s당 4파드/100%)도 무변경 — T+60 스텝 트래픽 즉응이 성능 12점 축이다.
- **기각**: `stabilizationWindowSeconds` 를 40 미만으로 — 회수 직후 재상승 구간에서 1↔N 왕복이 나면
  파드 기동 대기가 p99 를 깎아 성능 12점을 잃는다. 비용 12점을 벌려다 같은 크기를 내주는 거래다.
- **기각**: `consolidateAfter` 를 30s 미만으로 — 노드 기동이 ~2분이라 30초를 더 줄여 얻는 게 없고,
  짧은 계곡마다 노드를 지웠다 다시 띄우면 오히려 EC2 대수가 늘어난다.
- **주의**: 이 변경은 바닥이 아니라 꼬리만 자르지만 시간평균을 ratio 하한 0.50 쪽으로 민다.
  스케일아웃이 거의 없는 날에는 이 변경을 되돌리지 말고 HPA 목표치를 60% → 40% 로 먼저 내린다.
- **검증**: 로컬은 `kubectl apply --dry-run=client` 4개 파일 통과. 실측은 부하 종료 직후
  `kubectl get hpa -w` / `kubectl get nodeclaims -w` / Pending 0 확인 — 배포 후 몫.

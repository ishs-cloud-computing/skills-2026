...
### stress CPU limit — 별도 브랜치 (`task-3/stress-cpu-limit`)

위 "CPU limit 금지" 원칙의 유일한 예외 후보다. **기본 브랜치에는 넣지 않는다.** 병합하지 않고
두 변형을 다 쓸 수 있게 브랜치로 갈라 뒀다. 두 브랜치 차이는 `12-stress.yaml` 의 limit 한 줄뿐이다.

**왜 stress 만 예외인가**: HPA 와 Karpenter 는 포화 전에 파드를 퍼뜨린다 — stress 는 60% × 250m
= 150m, 실측으로 파드당 약 125 rps 에서 스케일아웃한다. 문제는 **노드 부팅 창 약 2분**이다.
그동안 stress 파드는 기존 노드에 묶여 갈 데가 없어 노드를 포화시킬 수 있고, 그때 user·product 의
0.2s 예산이 런큐 대기에 먹힌다. podAntiAffinity 는 이 창에서 무력하다 — 피할 노드가 아직 없다.
상한을 거는 것만이 유일한 방어다.

**왜 stress 는 스로틀링을 견디나**: 요청당 서버 처리시간이 100~250µs 인데 SLO 가 1.0s 다.
스로틀링이 무서운 건 처리가 무거운 앱이고 이 앱은 정반대다. user·product(0.2s SLO)에 같은
논리를 적용하면 안 된다.

**800m 의 근거**: 실측 193m @ 163 rps ≈ 요청당 1.2 millicore. 800m 이면 파드당 약 675 rps 까지
안 깎이고, 노드에 약 1100m 을 user·product·시스템 몫으로 남긴다. **이 숫자는 재구성 바이너리
기준이다** — 당일 바이너리로 요청당 millicore 를 다시 재고 비율만큼 조정한다(README STEP 12).

**해지선**: stress performance 90% 미만. 30% 가 아니다 — 90% 가 최상단 밴드(3-17)라 거기서
내려오는 순간부터 실점이고, 30% 를 깨면 stress 4점에 더해 cost ratio 12점 전부가 같이 날아간다
(4-1~4-12 가 3개 앱 전부 30% 이상을 요구). 해지 전에 상향(800m → 1200m)을 먼저 시도한다 —
완전 제거는 부팅 창 방어를 통째로 버리는 것이라 마지막 수단이다.

...
# - 프라이빗 서브넷 2AZ. IAM 인증은 항상 켠다(mark 4-3: Sasl.Iam.Enabled=True).
#   비인증 TLS(9094)는 producer_auth_mode=tls(기본)일 때만 켠다: 제공 producer 바이너리는
#   IAM signer 가 없고 9094 에서만 TLS 를 켠다(로컬 실행 테스트로 확정). iam 모드에서는
#   자체 IAM 바이너리를 쓰므로 unauthenticated=false 로 좁혀 IAM 전용 클러스터가 된다.

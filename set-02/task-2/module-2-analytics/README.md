# module-2-analytics — 실시간 주문 로그 분석 (ap-northeast-2)

프라이빗 EC2의 Flask 앱(gunicorn, systemd `app`)이 ALB 뒤에서 주문을 생성해 Kinesis로 전송, Managed Flink Studio Notebook(Zeppelin)에서 SQL로 실시간 분석.
...
PowerShell에서 JSON 인자는 내부 따옴표를 `\"`로 이스케이프해야 aws cli에 온전히 전달된다. `curl`은 PowerShell 별칭이므로 `Invoke-RestMethod`를 쓴다.
...
- 노트북 상태는 채점 시 **READY** — Run 상태(RUNNING)로 두면 2-4 오답. 시연 후 Stop 필수.

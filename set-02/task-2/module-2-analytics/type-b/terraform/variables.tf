...
# ON_DEMAND 스트림은 초기 4샤드다. parallelism 을 샤드 수에 1:1 로 맞춰
# 유휴 서브태스크(parallelism > 샤드)를 없앤다. 스트림이 리샤딩되면 같이 올린다.
variable "flink_parallelism" {
  description = "Managed Flink 처리 병렬도. Kinesis 샤드 수와 맞춘다 (ON_DEMAND 초기 4샤드)"
  type        = number
  default     = 4
}

variable "flink_parallelism_per_kpu" {
  description = "KPU 당 병렬 태스크 수 (기본 1, 최대 8). 1이면 KPU 수 = parallelism"
  type        = number
  default     = 1
}

...
- 리소스 이름은 과제지에 명시된 값과 **정확히 일치**(이름 일치 채점 항목 다수). 과제지 원문에 오타가 있어도 그대로 따르되, **정본이 바뀌면 정본을 따른다** — module-2 의 EC2 Role 이 그 사례다. 구판 원문 오타 `wsc2026-alaytics-ec2-role` 을 그대로 쓰고 있었는데 2026-08-21 신판이 `wsc2026-analytics-ec2-role` 로 고쳤다. 전사본(`task.md`)은 신판을 따랐고 **구현 리네임은 아직 안 됐다** — 이슈 #133.

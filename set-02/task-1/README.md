# set-02 / 제1과제 — wskorea26 Concert Platform
...
**작업 변수 영구화 (`~/.wskorea26-env`)** — 이후 모든 단계가 이 변수를 재사용한다.
터미널이 바뀌어도 이어서 작업할 수 있게 파일로 박아둔다 (cwd = `terraform`):

```bash
terraform output -json > ../outputs.json

cat > ~/.wskorea26-env <<EOF
export AWS_DEFAULT_REGION=ap-northeast-2
export NUM=$NUM
...
grep -qxF 'source ~/.wskorea26-env' ~/.bashrc || echo 'source ~/.wskorea26-env' >> ~/.bashrc
source ~/.wskorea26-env
```

### 2) 컨테이너 이미지 빌드 & ECR push (태그 `stable`)
...
안 되므로 mark.sh 는 S3 로 릴레이한다:

```bash
# [본 PC] (과제 루트 set-02/task-1 에서)
cd ..   # k8s/ 에 있었다면
aws s3 cp mark.sh "s3://$BUCKET/_transfer/mark.sh"
...
aws s3 cp "s3://$BUCKET/_transfer/mark.sh" ~/mark.sh && chmod +x ~/mark.sh
aws s3 rm "s3://$BUCKET/_transfer/mark.sh"   # 채점 전 정리 (mark 2-1 prefix 밖이지만 깔끔하게)
```
